...
public class Map_StudentCounter_II {
    public static void main(String[] args) {
        ...
        System.out.println("People who have got more candies than 4 candies: ");
        double sumAge = 0.0;
        for (Map<String, Object> rows : map) {
            if ((int) rows.get("candies") > 4) {
                System.out.println("\t" + rows.get("name"));
            }
            if ((int) rows.get("candies") < 5) {
                sumAge += Double.parseDouble(rows.get("age").toString());
            }
        }
